  ...
  async execute(interaction, client) {
    ...
    if (interaction.isButton()) {
      ...
      if (
        ...
        const id     = customId.replace("apply_approve_", "").replace("apply_reject_", "");
        ...
        await interaction.update({ components: disabledComponents }).catch(() => {});
        ...
        try {
          ...
          await user.send({
            embeds: [
              new EmbedBuilder()
                .setTitle(t(appLang, isApprove ? "commands.apply.approved" : "commands.apply.rejected"))
                .setColor(isApprove ? 0x2ecc71 : 0xe74c3c)
                .addFields(
                  { name: "ID",                                                       value: `\`${id}\``,                                       inline: true },
                  { name: t(appLang, "commands.apply.field_content"),                 value: app.content,                                       inline: true },
                  { name: t(appLang, "commands.apply.field_comment"),                 value: app.comment ?? t(appLang, "commands.apply.none") },
                )
                .setTimestamp(),
            ],
          });
        ...
        if (notifyType === "channel") {
          ...
          const row = new ActionRowBuilder().addComponents(
            new ChannelSelectMenuBuilder()
              ...
              .setPlaceholder(t(lang, "commands.apply.config_notify_placeholder"))
